...
while read -r name; do
 ...
 sed -i.bak -e 's/climb.eapaws.foliofn.com/uat.invest.climbinv/g' ../../../../dynamodb-records/$BRAND/$name.json
done
...
while read -r name; do
 ...
 sed -i.bak -e 's/verificationservice.uataws.foliofn.com/verificationservice.proddemo.foliofn.com/g' ../../../../dynamodb-records/$BRAND/$name.json
 ...
 sed -i.bak -e 's/uat.invest.climbinv/invest.climbinv/g' ../../../../dynamodb-records/$BRAND/$name.json
done
...
cp ../../../../dynamodb-records/$BRAND/*.json ../../../../dynamodb-records/$BRAND/prod
 ...
 if [ $TARGET='prod' ]
  then
    sed -i.bak -e 's/'$BRAND'.'$SOURCE'aws.foliofn.com/'$TARGET'.invest.collaboration.capital/g' ../../../../dynamodb-records/$BRAND/$TARGET/$name.json
 ...
 if [ $TARGET='uat' ]
 then
   sed -i.bak -e 's/'$BRAND'.'$SOURCE'aws.foliofn.com/'$TARGET'.invest.climbinv/g' ../../../../dynamodb-records/$BRAND/$TARGET/$name.json
 fi
